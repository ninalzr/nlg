...
class Attention(nn.Module):
    def __init__(self, encoder_size, decoder_size, device, type="coverage"):
        """ Attention module.         
            ...
            Notes: 
                Self-Attention(Intra-attention) Relating different positions of the same input sequence. Theorethically the self-attention can adopt any score functions above, but just replace the target sequence with the same input sequence.
    ...
    def forward(self, enc_output, state_h, mask=None):
        ...
        if mask is not None:
            mask = mask[:, :energy.size(1)] # this is required with pad-pack sequences because mask is longer than resulting sequences sometimes
